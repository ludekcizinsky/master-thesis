...
\begin{table}[!ht]
  ...
  \begin{tabular}{l
    ...
    E (SMPL-X params) & 77.5986 & {\bfseries 63.4009} & 0.6123 & {\bfseries 0.926} & 20.1 & 0.0809 \\
    \bottomrule
  \end{tabular}
...
\paragraph{Ablation setting.} Accurate pose estimation is crucial for realistic reconstruction of interacting humans. However, even current state-of-the-art methods struggle with recovering accurate pose when people interact closely. This motivated us to explore further pose tuning during 3DGS optimisation. Apart from improving the pose accuracy, our secondary motivation is to investigate whether improved pose also leads to better novel view synthesis. More precisely, if pose is incorrect, then the rendered 3DGS does not align well with the input image, making it difficult to learn accurate color and density values. Therefore, we hypothesize that improving pose accuracy should also improve novel view synthesis quality. To investigate this, we use our findings from the previous two ablations. Namely, we use LHM initialization and DiFix-based novel training view synthesis with 7 novel training cameras. We then adjust the training procedure such that we only optimise the first 15 epochs selected SMPL-X pose parameters based on the input monocular video, which we then freeze, generate novel training views using these updated poses, and then conduct another 15 epochs of 3DGS optimisation (as done before). The reason why we do pose and 3DGS optimisation separately is because as shown in the previous ablations, 3DGS needs multi-view supervision. To get this multi-view supervision, we need to use the estimated pose parameters to generate novel training views. Therefore, the moment we use these pose parameters for pseudo ground truth novel view generation, we need to freeze the pose parameters, otherwise we would be trying to update pose parameters however our ground truth signal would be signaling to change it back to the old values. To investigate the effect of pose optimisation, we start by setting a baseline which optimises root and body pose, translation and each person shape parameters. This method uses the same loss terms as the previous ablation studies.  We then propose three additional improvements of this baseline and investigate their effect which we describe next.

\paragraph{Interpenetration Loss.} First, we adopt the interpenetration loss introduced in Remips \cite{remips} where they applied it to naked human body models. In contrast, we adapt this term to our 3DGS human representation setting as follows. Given a set of gaussias for each person, we start by subsampling these points to make the computation feasible, by default, we use 2000 points per person. Given these points, we define the distance between two points $pA_i$ and $pB_j$ from person A and B as $d_{ij} = pA_i - pB_j$. We then compare this distance to an allowed margin computed as $m_{ij} = rA_i + rB_j + \delta$, where $rA_i$ and $rB_j$ are the radii of the Gaussians that generated the points, and $\delta$ is a small constant margin (by default set to 0.03m). The interpenetration loss is then defined as:
...
where $N_A$ and $N_B$ denote the number of subsampled Gaussian centers for person A and B.

\paragraph{Depth Order Loss.} Second, we adopt the depth order loss from MultiPly \cite{multiply}. In particular, we use the individual person masks from SAM3 as a guiding signal which of the persons in the scene is in front of the others and compare that to the rendered depths of each person from the 3DGS. If there is a disagreement between the rendered depth order and the SAM3 mask based depth order, we apply a penalty. More precisely, for every pixel $(u,v)$ where we have mask information based on the union of all person masks, we first obtain the \textit{rendered} depth of the person which is according to the mask in front, we denote this depth as $d_{\text{mask}}(u,v)$. We then compare the rendered depths of all persons for the given pixel and obtain the minimum depth among all persons, denoted as $d_{\text{min}}(u,v)$. In words, we pick the depth of the person which is in front according to the rendered depths. We then compute the depth order loss for given pixel as:

\begin{equation}
\mathcal{L}_{\text{depth}}(u,v) = \operatorname{softplus}\left(d_{\text{mask}}(u,v) - d_{\text{min}}(u,v)\right),
\end{equation}

where $\operatorname{softplus}(x) = \log(1+\exp(x))$. Finally, we average this loss over all pixels where we have mask information.

\paragraph{Confidence Guided Optimisation.} In general, it is not desirable to optimise our 3DGS model on frames where the pose estimation is poor. This is because we want to avoid adjusting 3DGS parameters to fit incorrect poses. Similarly, we also want to avoid a adjusting the shape parameter when pose in inaccurate. We therefore try to adapt MultiPly's confidence guided optimisation strategy \cite{multiply} which first splits the frames into reliable and unreliable according to the following criterion. We render each person in each frame and use the accumulated opacity to define the person's silhouette in 2D. We then compute the IoU between this silhouette and the corresponding SAM3 mask. We repeat this process for every person in the frame and average the IoUs to get a frame-level IoU score. If this score is above a certain threshold (by default set to 0.75), we consider this frame as reliable, otherwise unreliable. We update this reliability status every 5 epochs. During the pose optimisation phase, we only optimise pose and shape parameters on reliable frames, while we only optimise pose on unreliable frames. After the pose optimisation phase is done, we only use the reliable frames to generate novel training views for the subsequent 3DGS optimisation phase. And therefore completely discard unreliable frames for 3DGS optimisation. 

\begin{figure}[!ht]
    \centering
    \includegraphics[width=1.0\textwidth]{figures/qual_tune_and_notune_pose.drawio.png}
    \caption{\textbf{Qualitative comparison of using pose tuning}. First two columns show comparison of rendered 3DGS overlaid over the GT image. We highlight using red square box the biggest misalignment between the rendered image and the GT image due to inaccurate pose. The last two columns whos only the GT mesh (blue) and estimated mesh (orange).}
    \label{fig:qual_tune_and_notune_pose}  
\end{figure}


\paragraph{Interpretation of Pose Results.} Table~\ref{tab:ablation_pose_opt} summarises the results of our pose optimisation ablation study. First, we can see that pose tuning as expected improves MPJPE and MVE compared to no pose tuning. This confirms our hypothesis that optimising pose during 3DGS training can improve pose accuracy. However, we can also see that PCDR slightly degrades compared to no pose tuning. Second, we can see that despite adding different loss terms or optimisation strategies, we do not manage to obtain significant improvement compare to the naive baseline (section A). The only exception to this is when we add to the set of optimised SMPL-X parameters also both hands which yields the best MVE. For the interpenetration loss, we hypothesise that the key bottleneck might be our subsampling of gaussians. As a result, we might be missing some interpenetration cases which means that the loss is not effective enough. For the depth order loss, possible bottleneck can be small errors in the SAM3 masks which can lead to incorrect depth order supervision. 


\paragraph{Interpretation of the Novel View Synthesis Results.} We can see that overall, pose optimisation does not lead to significant improvements in novel view synthesis quality compared to no pose tuning. The only large improvement is visible with the LPIPS score suggesting that improved pose quality leads to possibly sharper renderings. Last but not the least, following MultiPly's evaluation protocol, we use the ground truth pose to synthesise novel views for evaluation. Therefore, the improved pose quality can be only beneficial during training. This is demonstrated in Figure~\ref{fig:qual_tune_and_notune_pose} where we show a qualitative comparison between using pose tuning and not using pose tuning. We can see that using pose tuning leads to better alignment between the rendered 3DGS and the GT image. This better alignment during training can help the model to learn better color and density values which can lead to improved novel view synthesis quality even when evaluated with GT poses.
