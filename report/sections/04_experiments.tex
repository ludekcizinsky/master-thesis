...
\paragraph{Ablation setting.} Accurate pose estimation is crucial for realistic reconstruction of interacting humans, yet it remains challenging in close interactions. We therefore explore pose tuning during 3DGS training and evaluate its effect on both pose accuracy and novel view synthesis (NVS).

We use our best settings from the previous ablations (LHM initialization and DiFix-based NVS with 7 novel training cameras) and follow a two-stage schedule: (i) 15 epochs of pose tuning on the monocular video, (ii) freeze the tuned SMPL-X parameters, synthesize novel training views using the updated poses, and (iii) 15 epochs of 3DGS optimization as in the main pipeline (see Method for details). Section A in Table~\ref{tab:ablation_pose_opt} tunes \texttt{root\_pose}, \texttt{body\_pose}, \texttt{trans} and \texttt{betas}; Sections B--D add the tested loss/optimization components; and Section E expands the optimized SMPL-X parameter set by adding hands. We describe each component in the paragraphs below.

\paragraph{Interpenetration Loss.} First, we adopt the interpenetration loss introduced in Remips \cite{remips}, where it is applied to naked human body models. In contrast, we adapt this term to our 3DGS human representation as follows. Given a set of Gaussians for each person, we subsample their centers to make the computation feasible (by default, we use 2000 points per person). For two people A and B, and their subsampled Gaussian centers $\{pA_i\}_{i=1}^{N_A}$ and $\{pB_j\}_{j=1}^{N_B}$, we define the displacement vector between a pair of centers as $d_{ij} = pA_i - pB_j$ and the corresponding distance as $\lVert d_{ij} \rVert_2$. We compare this distance to an allowed margin $m_{ij} = rA_i + rB_j + \delta$, where $rA_i$ and $rB_j$ are the radii of the two Gaussians and $\delta$ is a small constant margin (by default set to 0.03\,m). The interpenetration loss is then defined as:
...
\paragraph{Interpretation of Pose Results.} Table~\ref{tab:ablation_pose_opt} summarises the results of our pose optimisation ablation study. Pose tuning improves MPJPE and MVE compared to no pose tuning, confirming that optimizing pose during 3DGS training can improve pose accuracy. At the same time, the best PCDR is achieved by the \textit{No Pose Tuning} setting. This suggests that our pose tuning improves joint/vertex accuracy on average, but does not necessarily improve (and can slightly hurt) PCDR, which may be sensitive to segmentation noise and small depth inconsistencies.
...
Converting the optimized 3DGS representation to a mesh is a crucial step for geometry evaluation, yet it is not trivial. We use the training pipeline outlined in the Method section for the mesh extraction experiments, and explore two strategies for converting the 3DGS to a mesh in this ablation study: (i) Marching Cubes (MC) \cite{marching_cubes} and (ii) Truncated Sign Distance Field Fusion of the rendered depth maps as implemented in Open3D \cite{open3d}.  


\paragraph{3D Voxel Grid Definition from 3DGS.} In order to use MC, we first need to convert the 3DGS to a 3D voxel grid with occupancy values for each point in the 3D grid. To define the occupancy value for each of these points, we need to first compute contribution of each Gaussian to the point as follows:
...
\begin{figure}[!t]
    \centering
    \includegraphics[width=0.9\textwidth]{figures/qual_3dgs_to_mesh.drawio.png}
...
\paragraph{Interpenetration of the results.} Table \ref{tab:ablation_3dgs_to_mesh} shows a clear trend across both datasets where TSDF fusion achieves better results across all metrics except from Volumetric IoU (V-IoU), where MC performs better. Figure \ref{fig:qual_abl_3dgs_to_mesh} further supports these results quallitatively where TSDF fusion produces smooth non-blobby meshes compare to the MC results. In addition, MC also shows many holes in the extracted meshes. Last but not the least, we can also see results get consistently worse when moving from Hi4D to MMM which has more people. We hypothesise that this drop in performance is mainly due to worse pose estimation which directly affects the extracted mesh since they are based on the posed 3DGS (which depends on the SMPL-X pose). Overall, these results confirm that TSDF fusion is a better choice for converting 3DGS to mesh compared to MC which is the reason we used TSDF fusion for the reconstruction results in Table \ref{tab:reconstruction_results}.