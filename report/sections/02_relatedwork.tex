...
Human mesh recovery (HMR) methods estimate the parameters of a parametric body model from images or videos. A useful distinction is between local HMR, which predicts pose and shape in a per-frame camera coordinate system from single images, and global HMR, which aims to recover temporally consistent motion and camera-aware trajectories in a shared world frame from monocular video \cite{slahmr}.
