...
Several extensions improve expressiveness. SMPL-H adds articulated hands \cite{smplh}, and SMPL-X further integrates detailed hands and a face model based on FLAME \cite{smplx,flame}. More recent work targets limitations of the training data distribution of SMPL-style models, for example by improving population diversity \cite{annybody}, or by decoupling skeletal pose from body shape for more controllable rigs \cite{yang2025sam3dbody}. In our work, we use SMPL-X because it is predicted by our chosen Human Mesh Recovery (HMR) method and provides a convenient parameterization to drive deformations of our explicit 3D representation, while supplying a strong geometric prior on human shape.


\section{Human Motion Estimation}
HMR methods estimate the parameters of a parametric body model from images or videos. A useful distinction is between local HMR, which predicts pose and shape in a per-frame camera coordinate system from single images, and global HMR, which aims to recover temporally consistent motion and camera-aware trajectories in a shared world frame from monocular video \cite{slahmr}.
...
Early approaches estimate per-frame local HMR and then lift results to global motion with additional optimization, for example by estimating global translation and refining temporally \cite{glamr}. Subsequent work incorporates Simultaneous Localization and Mapping (SLAM) to obtain camera motion and improve global consistency \cite{slahmr,tram}. More recent methods regress global body model parameters end-to-end from monocular video, often using memory or recurrence to exploit temporal context \cite{wham,trace}. Despite strong progress, many methods still depend on multiple off-the-shelf estimators (for example, 2D keypoints, segmentation, and depth), which can harm temporal stability when these components fail \cite{chen2025human3r}. We use Human3R \cite{chen2025human3r} because it jointly reasons about humans and scene geometry and provides camera and body parameters in a consistent shared frame, which is important for our downstream reconstruction.


\section{General 3D and 4D Reconstruction from Dense Input Capture}
...
\section{Human-centric 4D Reconstruction}
We group methods here as \emph{human-centric} because they target or extensively evaluate on human performance capture, which is particularly challenging due to articulated motion, frequent self-occlusions, and fine-scale dynamics such as clothing and hair. 

\subsection{Dense Input Capture}
In dense input capture settings, the multi-view setup often provides sufficient constraints without explicit human priors, making reconstruction more tractable. As a result, work in this category emphasizes not only reconstruction quality but also whether the approach can run in real time. An early method addressing real-time reconstruction of dynamic scenes with complex human motion from multi-view RGB-D video was Fusion4D \cite{fusion4d}. Later, Lombardi et al. drop the depth assumption and introduce a neural representation called Neural Volumes, using an encoder-decoder network to predict, for each frame, an explicit volumetric scene representation from the corresponding multi-view image set \cite{neuralvolume}. Their approach captures complex non-rigid deformations, including clothing and hair. Besides requiring a network query per frame, a key limitation of this approach is that higher output resolution leads to a substantial increase in memory consumption. Consequently, recent work has shifted toward more efficient and explicit 4D representations that extend the successful 3DGS formulation to dynamic scenes. A recent representative is Free Time GS \cite{freetimegs}, which extends 3DGS to 4D by allowing Gaussians to appear and disappear over time, enabling the modeling of complex non-rigid motion and topology changes. While these methods achieve impressive results, their real-world applicability is limited due to the need for multi-view video capture. This motivates our focus on monocular video input, which we address by leveraging strong human priors and generative view synthesis to mitigate the challenges of sparse input.

\subsection{Sparse Input Capture}
In contrast, when input is sparse (often monocular), leveraging prior knowledge about human body shape and motion becomes crucial to compensate for missing views and ambiguities.

\paragraph{Single person reconcstruction from monocular video.} Early methods such as HumanNeRF \cite{humannerf}, NeuMan \cite{neuman}, and Vid2Avatar \cite{vid2avatar} reconstruct implicit neural representations of a single human from monocular video. While they can produce high-quality results, implicit representations typically require expensive per-subject optimization and can be slow to render. In addition, another disadvantage of the implicit representation is its poor composability. This motivated explicit Gaussian-based human avatars \cite{drivable3dgs,gauhuman,gaussianavatar,exavatar}, which improve efficiency and are easier to integrate into rasterization-based pipelines. However, monocular input still provides limited coverage of the subject, especially for in-the-wild videos with occlusions and fast motion. MV-Performer \cite{mvperformer} addresses view sparsity by leveraging multi-view video diffusion for view synthesis, but it focuses on single-person scenes.  In contrast to these works, our method targets multi-person scenes and accounts for interactions and occlusions between people.

\paragraph{Single person reconstruction from sparse images.} Similar to these monocular-video-based approaches, there is an increasing body of work focusing on an even more restricted input setting. Specifically, these approaches aim to reconstruct a canonical human from a single image or a sparse set of images. Some methods use video diffusion models to synthesize additional canonical views \cite{anigs,gas}. LHM \cite{qiu2025lhm} instead predicts a canonical 3DGS avatar in a feed-forward manner that can be animated using SMPL-X. We use LHM for per-person initialization because it provides fast and high-quality canonical reconstructions. However, it was designed for single-person inputs and does not estimate motion, so multi-person scenes can require additional refinement to avoid interpenetrations and to recover details that are not visible in the initialization view.

\paragraph{Multiple persons methods.} Compared to single-person settings, the literature on multi-person 4D reconstruction from sparse input is more limited. Guess the Unseen \cite{gtu} addresses view sparsity by fine-tuning a diffusion model per person and using it to synthesize additional views, which can be time consuming and scales poorly with the number of people. MultiPly \cite{multiply} tackles multi-person interactions and occlusions by optimizing an implicit signed distance field representation, but reports high computational cost due to per-person optimization. Our work follows the same goal of reconstructing interacting people in monocular video, but uses an explicit 3DGS representation for efficiency and introduces generative priors via DiFix \cite{wu2025difix3d} to mitigate the sparse-view setting.
