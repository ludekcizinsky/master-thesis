...
% why this section: in order to be able to talk about 4d reconstruction, we first need to review 3d reconstruction methods on top of which 4d methods are built

% some other importnat papers that i yet do not understand fully:
% nerfies, hypernerf, reconfusion




% static sparse view 3d reconstruction: 
% - methods that rely on learned data priors to make the monocular setting better constrained: mv splat, grm, gs-lrm 


% what was before nerf:
% - colmap and the paper structure from motion revisited (2016)
% - before nerf, you would use dense capture and the idea of light fields to reconstruct 3d scenes from images 
...
% Works that primarily focus on the 3d/4d representation under the assumption of dense capture
% --------------------
% 3d methods:
% - task definition: 3d reconstruction: estimate geometry and apperance of the scene from a set of images
% - classical style: depth maps + and feature matching to arrive at some form of point cloud
% - optimisation based that use some neural represenation: nerf, 3dgs
...
% - emphasis put on modelling motion: shape of motion, mosca, 4dgs, Gaussian flow, free time gs
% - general scene and SLAM like approach: mega sam - idea combine feed forward depth estimation with with optimisation based post recosntruction of the scene
