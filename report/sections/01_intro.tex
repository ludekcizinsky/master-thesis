...
In parallel, there is a growing line of feed-forward reconstruction models that map monocular video directly to a 3D/4D representation without per-scene optimization, which is closer in spirit to Sutton's lesson. While these models promise high throughput (appealing, for example, in robotics), they still lag behind optimization-based methods in terms of the reconstruction quality, and this is especially true in challenging dynamic scenes.

A key bottleneck for feed-forward 4D reconstruction methods is training data, which remains scarce for real-world dynamic scenes. This motivates hybrid methods that exploit pretrained priors while still performing targeted per-scene refinement at a practical runtime. In this thesis, we explore one such hybrid pipeline for human-centric 4D reconstruction from monocular video. We hope that apart from enabling new exciting applications, it is also a potential step towards producing higher-quality dynamic 4D supervision for training stronger feed-forward models in the future.
...
Besides parametric-model-based pipelines (SMPL \cite{smpl} and SMPL-X \cite{smplx}), there are also more general approaches such as Shape-of-Motion \cite{som}, which use pretrained tracking models to define deformation fields without an explicit human body prior. While these methods are appealing for their generality, the resulting humans often have weaker pose and shape quality than approaches that explicitly constrain motion with a parametric body model. Finally, feed-forward reconstruction methods remove per-scene optimization entirely and map monocular video directly to a 4D representation, trading accuracy for throughput. In this work, we aim for a practical middle ground: a human-centric pipeline that retains strong priors and achieves competitive rendering quality in controlled evaluations, while keeping end-to-end runtimes in the tens-of-minutes regime in our setup rather than hours or days. Against this background, we summarize our main contributions below.
...
\begin{itemize}
    ...
    \item An experimental evaluation across pose estimation, novel view synthesis, mesh reconstruction, and segmentation on Hi4D and MMM, highlighting strengths and remaining limitations of the approach.
\end{itemize}
