...
In addition to shape and expression blendshapes, the standard SMPL-X formulation includes pose-dependent blendshapes $B_{p}(\theta)$, which model non-linear pose effects such as muscle bulging. These offsets are applied in the canonical space before skinning. We rely on the standard SMPL-X implementation \cite{smplx} for these details.
...
\begin{figure}[!ht]
    ...
    \caption{Overview of our novel training view synthesis pipeline using DiFix. We start with the monocular video and the estimated SMPL-X parameters to render initial novel views of the humans using the canonical 3DGS obtained from LHM during preprocessing. These initial renderings are then refined using DiFix to improve multi-view and temporal consistency. The refined novel views are then used as input to our main training pipeline.}
    \label{fig:novel_view_synthesis_difix}  
\end{figure}


Our explicit scene representation is sensitive to the number of input views and can overfit to the observed views, which leads to blur and holes when rendering from unseen viewpoints. To mitigate this, we lift the monocular video to a multi-view training set by synthesizing novel training views around the scene. The resulting supervision is dense in time (we synthesize full videos for each virtual camera) but still sparse in viewpoints (we use a small number of virtual cameras). The key challenge is to generate videos that are both multi-view consistent and temporally stable.

Figure \ref{fig:novel_view_synthesis_difix} summarizes this procedure. We generate a set of virtual cameras on a circle around the reconstructed subject and render an initial estimate of each novel view using the canonical 3DGS initialization from LHM together with the estimated SMPL-X parameters and camera parameters. These renderings are imperfect but provide a strong geometric and appearance prior that reduces ambiguity for view synthesis.

To improve visual quality and enforce consistency, we refine the initial renderings using DiFix \cite{wu2025difix3d}, an image-to-image diffusion model. DiFix takes (i) a reference image, (ii) an image to refine, and (iii) a text prompt. We use the default prompt from the original work (``remove degradation''). For each target camera and each frame, we render the initial novel view and refine it with DiFix using a nearby reference view. We repeat this procedure for all frames, producing a temporally consistent novel view video for each virtual camera. The resulting refined videos are then treated as pseudo ground truth training views and used as additional supervision during 3DGS optimization.

\begin{figure}[!ht]
    \centering
    \includegraphics[width=0.8\textwidth]{figures/trn_view_cam_gen_in_3d.png}
    \caption{Visualisation of the generated training view cameras around the original scene. The generated cameras are in orange, while the original camera trajectory is in blue. We use Viser \cite{yi2025viser} to obtain this visualisation.}
    \label{fig:trn_nv_cam_gen_3d_vis}  
\end{figure}

Figure \ref{fig:trn_nv_cam_gen_3d_vis} illustrates the placement of the virtual training cameras around the scene relative to the source camera trajectory.
