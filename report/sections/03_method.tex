...
\begin{equation}
  ...
  \right)
\end{equation}
In words, we assign higher weights to Gaussians that are closer to the pixel center $\mathbf{u}$, with the falloff determined by the projected covariance $\boldsymbol{\Sigma}_{i}^{\mathrm{2D}}$. Before computing the final pixel color, we sort the Gaussians along the camera ray based on their depth (distance from the camera). We then obtain color for each color using front-to-back alpha compositing as follows:
...
\paragraph{Making use of rendering library.} In practice, we use gsplat library \cite{ye2025gsplat} to handle the whole rendering process, i.e. both the projection and rasterization steps. importantly, the whole process is differentiable, which allows us to optimize the 3D Gaussian parameters.
